import * as React from 'react';
import CardGym from './components/cardGym';
import './savedWorkouts.css'
function SavedWorkouts() {
    return (
        <div className='saved-wo'>
            <div class='saved-wo-title'>
            <h2>Your Saved Workouts</h2>
            </div>
            <div className='all-cards'>
                <div className='card1'>
                    <CardGym className='card1-c'
                    title='o'
                    body='Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo.
                    Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo'
                    />                
                </div>
                <div className='card2'>
                    <CardGym className='card2-c'
                    title='benny'
                    body='Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo.
                    Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo'
                    />
                </div>
                <div className='card3'>
                    <CardGym className='card3-c'
                    title='e'
                    body='Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo.
                    Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo'
                    />                
                </div>
                <div className='card4'>
                    <CardGym className='card4-c'
                    title='o'
                    body='Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo.
                    Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo'
                    />
                </div>
                <div className='card5'>
                    <CardGym className='card5-c'
                    title='perdigao'
                    body='Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo.
                    Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo'
                    />                
                </div>
                <div className='card6'>
                    <CardGym className='card6-c'
                    title='curtem'
                    body='Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo.
                    Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo'
                    />
                </div>
                <div className='card7'>
                    <CardGym className='card7-c'
                    title='de'
                    body='Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo.
                    Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo'
                    />                
                </div>
                <div className='card8'>
                    <CardGym className='card8-c'
                    title='vareta'
                    body='Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo.
                    Vítor Campelos, treinador do Chaves, considera que o empate no terreno do Vizela (2-2), na entrada em cena na fase de grupos da Taça da Liga, foi inteiramente justo'
                    />
                </div>
            </div>    
        </div>
    )
}

export default SavedWorkouts